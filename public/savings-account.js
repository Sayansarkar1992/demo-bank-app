...
function validateTxnInput() {
  ...
  return {
    ...
    payload: {
      amount,
      narration: narration || 'Manual transaction from savings page'
    }
  };
}

async function loadSavings() {
  const res = await fetch('/api/accounts/savings', { headers: { ...authHeaders() } });
  if (!res.ok) {
  ...
  setText('rate', data.interestRate);

  const body = document.getElementById('txBody');
  body.innerHTML = data.transactions
    .map(
      (tx) => `
      <tr data-testid="savings-tx-${tx.id}">
        <td>${tx.id}</td>
        <td>${tx.date}</td>
        <td>${tx.type}</td>
        <td>${tx.amount}</td>
...
async function downloadPassbook() {
  const res = await fetch('/api/accounts/savings/passbook', {
    headers: { ...authHeaders() }
  });
  ...
  a.href = url;
  a.download = 'savings-passbook.csv';
...
document.getElementById('depositBtn').addEventListener('click', async () => {
  await postTransaction('/api/accounts/savings/deposit');
});

document.getElementById('withdrawBtn').addEventListener('click', async () => {
  await postTransaction('/api/accounts/savings/withdraw');
});
